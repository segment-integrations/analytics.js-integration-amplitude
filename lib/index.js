...
function logEvent(track) {
  ...
  if (track.revenue()) this.setRevenue(mapRevenueAttributes(track));
}
...
Amplitude.prototype.orderCompleted = function(track) {
  ...
  var trackWithoutProducts = new Track(clone(track).obj);
  delete trackWithoutProducts.obj.properties.products;
  ...
  // If the user has chosen the second option, we need to delete the revenue and total properties. 
  // from this event to prevent revenue from being double counted.
  // .revenue is used to decide whether or not Amplitude Revenue should be logged.
  // track.revenue() will fall back on total so we need to delete that as well.
  if (trackRevenuePerProduct) {
    delete trackWithoutProducts.obj.properties.revenue;
    delete trackWithoutProducts.obj.properties.total;
  }

  // Log the top level Order Completed event.
  logEvent.call(this, trackWithoutProducts);
  ...
  each(function(product) {
    ...
    if (price != null && quantity) {
      if (trackRevenuePerProduct) {
        this.setRevenue(mapRevenueAttributes(new Track({ event: 'Order Completed', properties: product })));
      } else {
        // Revenue should not be a property of a product in an Order Completed event.
        // If someone does add it though, we need to delete it here before we trigger a 
        // Product Purchased event. Otherwise, we would double count revenue.
        delete product.revenue;
        var trackWithoutRevenue = clone(track);
        trackWithoutRevenue.obj.event = 'Product Purchased';
        trackWithoutRevenue.obj.properties = product;
        logEvent.call(this, new Track(trackWithoutRevenue.obj));
      }
    }
  }.bind(this), products);
};
